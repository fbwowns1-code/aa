...
키워드 하나를 입력하면:
  ...
  3) 한글 인포그래픽 썸네일 프롬프트 생성 (턴3) → 챗지피티 웹채팅(기본) 또는
     이미지 생성 API로 실제 이미지 파일 제작 (실사 이미지 프롬프트는 쓰지 않는다)
...
def main():
    ...
    turn1 = pipeline.run_turn1(args.keyword, args.reference, IMAGE_MODE, args.extra)
