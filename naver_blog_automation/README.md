...
**중요한 제약**: 이 화면에서 누른 실행은 항상 **완전 자동 모드**로 돈다 — 기획팀이 제목 1번을 자동으로 채택하고, 중간에 "이 부분 고쳐줘" 같은 대화형 수정은 할 수 없다(웹 버튼은 터미널 입력을 받을 방법이 없기 때문이다). 턴마다 결과를 보면서 대화로 고치고 싶으면 터미널에서 `python main.py`를 직접 실행한다(위 "지시 방법 1" 참고) — 그 경우에도 부서별 진행 현황 패널은 같은 `output/status.json`을 보므로, 터미널로 돌리는 동안 웹 화면을 열어서 진행 상황만 구경할 수도 있다.
