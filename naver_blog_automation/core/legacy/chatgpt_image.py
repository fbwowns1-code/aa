"""
[LEGACY/FALLBACK] 챗지피티 웹채팅(chatgpt.com)에 "한글 인포그래픽 썸네일
프롬프트"를 붙여넣어 이미지를 생성시키고, 다운로드해서 파일로 저장한다.

기본 이미지 생성 경로는 이제 core/image_gen.py(OpenAI 이미지 생성 API)로
통일됐다. 이 모듈은 image_provider="chatgpt_web"으로 명시적으로 설정했을
때만 core/job_manager.py가 골라서 불러온다 — core/legacy/ 아래로 옮긴 건
기능을 지운 게 아니라 "기본 경로가 아님"을 명확히 하기 위해서다.
